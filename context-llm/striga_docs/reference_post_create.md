...
source_url: https://docs.striga.com/reference/post_create
scraped_at: 2025-07-18 17:51:45
---
...
post https://www.sandbox.striga.com/api/v1/standing-orders/create

Create a standing order that takes in a user ID (must be APPROVED), a currency, network and address. This will send an OTP to the user after all checks pass, to be entered into the "Confirm Standing Order" API, after which, any fiat funds deposited into the users' account are automatically converted into the chosen currency and sent. A single EUR account is denominated as the source for this standing order. This way, other wallets/accounts used for regular deposit/trading/spending functionality is left untouched. Any EUR deposits arriving into the denominated account ID will be converted into the currency denominated in the whitelisted address and paid out with the then prevailing network fees.
